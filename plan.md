...
## 23. Causa raíz real del "LIMIT 1" — encontrada y corregida (2026-08-22)

Versión `1.20.2`. El bug de la sección 20/21 no era mío: `Db::getValue()`/`getRow()` de PrestaShop **agregan `LIMIT 1` automáticamente a cualquier consulta**, y `upgrade/upgrade-1.0.3.php` (script viejo, de la versión 1.0.3, sigue empaquetado porque cualquier sitio con `ps_module.version` desincronizado necesita poder re-ejecutar todo el historial) hacía `$db->getValue('SHOW COLUMNS FROM ... LIKE ...')` — y `SHOW COLUMNS ... LIMIT 1` es sintaxis inválida en el MariaDB de este hosting. **Fix**: usar `executeS()` en vez de `getValue()` (no le agrega `LIMIT`).

Por qué solo aparecía ahora: `wephone.es` y `saruia.es` se desplegaron siempre por FTP manual, nunca completaron un `runUpgradeModule()` real — así que `ps_module.version` en base de datos quedó atascado en un valor viejo, y CUALQUIER intento de actualización (aunque fuera de 1.19.3 a 1.20.0) forzaba a PrestaShop a re-ejecutar los 30+ scripts de upgrade desde el principio, tropezando siempre con el mismo bug de 1.0.3 antes de llegar a nada nuevo.

**Cómo se encontró de verdad**: los reintentos vía "Actualizar ahora" no servían para diagnosticar porque el código de logging nuevo vive en la versión que se está instalando, no en la que corre — el propio proceso que falla usa el código VIEJO en memoria (el problema del huevo y la gallina descrito en la sección 21, llevado a su extremo). Se resolvió llamando `Module::needUpgrade()`/`runUpgradeModule()` **directamente vía un script de diagnóstico puntual** (mismo patrón de siempre: token-gated, sube por SFTP/FTP, se borra después) contra el código ya desplegado — eso sí devolvió la traza completa con archivo y línea exactos.
