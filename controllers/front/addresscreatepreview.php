...
require_once dirname(__FILE__) . '/../../classes/AiBridgeAddressCreatePreview.php';
...
class AibridgeAddresscreatepreviewModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
        ...
        try {
            ...
            if (!$preview['valid']) {
                $this->sendJson(400, array(
                    'success' => false,
                    'error' => array('code' => 'invalid_payload', 'message' => 'Invalid address creation payload.'),
                    'errors' => $preview['errors'],
                ));
            }

            $employeeId = $this->module->getAuthenticatedEmployeeId();
            $request = AiBridgeApprovalRequest::createPendingAddressCreate(
                $preview,
            ...
            $this->sendJson(200, array(
                ...
                'data' => array(
                    ...
                    'status' => $status,
                    'applied' => $applied,
                    'created_address_id' => $applied ? (int) $request->created_product_id : null,
                ),
            ));
        } catch (\Throwable $exception) {
            $this->sendJson(500, array(
                'success' => false,
                'error' => array('code' => 'address_create_preview_internal_error', 'message' => 'Address creation preview could not be generated.'),
            ));
        }
    }
