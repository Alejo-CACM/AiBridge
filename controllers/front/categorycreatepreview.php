...
require_once dirname(__FILE__) . '/../../classes/AiBridgeCategoryCreatePreview.php';
...
class AibridgeCategorycreatepreviewModuleFrontController extends ModuleFrontController
{
        ...
        try {
            ...
            if (!$preview['valid']) {
                $this->sendJson($preview['conflict'] ? 409 : 400, array(
                    'success' => false,
                    'error' => array(
                        'code' => $preview['conflict'] ? 'duplicate_category' : 'invalid_payload',
                        'message' => $preview['conflict'] ? 'Category link rewrite already exists.' : 'Invalid category creation payload.',
                    ),
                ));
            }

            $employeeId = $this->module->getAuthenticatedEmployeeId();
            $request = AiBridgeApprovalRequest::createPendingCategoryCreate(
                $preview,
            ...
            $this->sendJson(500, array(
                'success' => false,
                'error' => array('code' => 'category_create_preview_internal_error', 'message' => 'Category creation preview could not be generated.'),
            ));
        }
    }
